...
class ProgressBannerWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        self.status_label.setStyleSheet(
            "font-size: 11pt; font-weight: bold; background: transparent; color: #0D47A1;"
        )
        ...
        self.progress_label.setStyleSheet(
            "font-size: 10pt; background: transparent; color: #37474F;"
        )
        ...
        self.current_file_label.setStyleSheet(
            "font-size: 9pt; background: transparent; color: #546E7A;"
        )
    ...
    def show_completion(self, success: bool, message: str):
        ...
        if success:
            ...
            self.status_label.setStyleSheet(
                "font-size: 11pt; font-weight: bold; background: transparent; color: #2E7D32;"
            )
        else:
            self.icon_label.setText("⚠")
            self.status_label.setText(message)
            self.status_label.setStyleSheet(
                "font-size: 11pt; font-weight: bold; background: transparent; color: #F57C00;"
            )
...
class FinalConfirmationDialog(QDialog):
    ...
    def _init_ui(self, source_paths, expected, actual, searchable):
        ...
        summary = (
            f"PDF Created: <b>{os.path.basename(self.pdf_path)}</b><br>"
        ...
        if expected != actual:
            warning_label = QLabel(
                f"<p style='color:red; font-weight:bold;'>⚠️ VERIFICATION FAILED! Expected {expected} pages, but the PDF has {actual} pages.</p>"
            )
            warning_label.setTextFormat(Qt.TextFormat.RichText)
            main_layout.addWidget(warning_label)

        if not searchable:
            searchable_warning_label = QLabel(
                "<p style='color:orange; font-weight:bold;'>ⓘ WARNING: This PDF is image-only and NOT text-searchable.</p>"
            )
...
class PagePreviewWidget(QWidget):
    def __init__(self, image_path, thumbnail_width=100, parent=None):
        ...
        scaled_pixmap = pixmap.scaledToWidth(
            thumbnail_width, Qt.TransformationMode.SmoothTransformation
        )
...
class ConvertPDFsWindow(QMainWindow):
    ...
    def _update_selection(self):
        ...
        self.selected_pdfs = [
            cb.property("pdf_path") for cb in self.pdf_checkboxes if cb.isChecked()
        ]
    ...
    def _extract_pdfs(self):
        ...
        for idx, pdf_path in enumerate(self.selected_pdfs):
            ...
            try:
                ...
                self.extraction_results.append(
                    {"pdf_name": pdf_name, "success": False, "error": str(e)}
                )
...
class MetadataDisplayWidget(QWidget):
    ...
    def _create_analysis_card(self) -> QWidget:
        ...
        title_label.setStyleSheet(
            "font-size: 12pt; font-weight: bold; color: #111827; background: transparent; border: none;"
        )
        layout.addWidget(title_label)
        ...
        self.doc_type_label.setStyleSheet(
            "font-size: 10pt; color: #374151; background: transparent; border: none;"
        )
    ...
    def _create_bundle_card(self) -> QWidget:
        ...
        self.bundle_count_label.setStyleSheet(
            "font-size: 10pt; color: #6B7280; background: transparent; border: none;"
        )
    ...
    def _create_bundle_thumbnail_widget(self, file_path: str) -> QWidget:
        ...
        filename_label.setStyleSheet(
            "font-size: 9pt; color: #111827; background: transparent; border: none;"
        )
        filename_label.setWordWrap(True)
    ...
    def set_current_file(self, file_path: str):
        ...
        if rotation_degrees and rotation_degrees != 0:
            ...
            self.rotation_label.setStyleSheet(
                "font-size: 10pt; color: #059669; background: transparent; border: none;"
            )
    ...
    def set_bundle_files(self, file_paths: list[str]):
        ...
        for file_path in file_paths:
            ...
            self.bundle_thumbnails_layout.insertWidget(
                self.bundle_thumbnails_layout.count() - 1, thumbnail_widget
            )
...
class ConvertImagesWindow(QMainWindow):
    ...
    def __init__(self, parent=None, config_manager=None, analysis_db=None, metadata_db=None):
        ...
        # Workflow state
        self.current_step = (
            WorkflowStep.BUNDLE_SUGGESTIONS
        )  # Phase 7: Start with bundle suggestions
    ...
    def _init_ui(self):
        ...
        self.header_back_button.setIcon(
    ...
    def _setup_zoom_controls(self):
        ...
        zoom_container.move(
            self.large_preview_label.width() - 210, self.large_preview_label.height() - 85
        )
    ...
    def resizeEvent(self, event):  # noqa: N802
        ...
        if hasattr(self, "zoom_mode") and self.zoom_mode in [
            "fit_width",
            "fit_height",
            "fit_window",
        ]:
    ...
    def _shortcut_approve_continue(self):
        ...
        if self.current_step == WorkflowStep.STITCHING:
            ...
            if (
                hasattr(self, "exclude_button")
                and self.exclude_button.isVisible()
                and self.exclude_button.isEnabled()
            ):
                self.exclude_button.click()
        elif self.current_step == WorkflowStep.ORDERING:
            # Approve order button in Step 3
            if (
                hasattr(self, "approve_order_button")
                and self.approve_order_button.isVisible()
        ...
        for i in range(self.thumbnail_layout.count()):
            ...
            if item and item.widget():
                ...
                if hasattr(widget, "file_path") and widget.file_path == file_path:
                    ...
                    # Reset after duration
                    QTimer.singleShot(
                        duration, lambda w=widget, s=original_style: w.setStyleSheet(s)
                    )
                    break
    ...
    def _start_auto_approval(self, button, button_text):
        ...
        delay = int(
            self.config_manager.get_setting("AutoApproval", "automatic_approval_delay", "5")
        )
    ...
    def _update_auto_approval_countdown(self):
        ...
        if self.auto_approval_countdown > 0:
            # Update button text
            self.auto_approval_button.setText(
                f"{self.auto_approval_original_text} ({self.auto_approval_countdown})"
            )
        else:
    ...
    def _apply_initial_splitter_sizes(self):
        ...
        self.content_splitter.moveSplitter(
            left_width, 1
        )  # Move first handle to left_width position
    ...
    def _update_loading_spinner(self):
        ...
        if hasattr(self, "loading_spinner"):
            ...
            self.loading_spinner_index = (self.loading_spinner_index + 1) % len(
                self.loading_spinner_frames
            )
    ...
    def _setup_step1_ui(self):
        ...
        self.image_gallery = ImageGalleryWidget(
            analysis_db=self.analysis_db, parent=self.left_panel
        )
        ...
        self.metadata_display = MetadataDisplayWidget(
            analysis_db=self.analysis_db, parent=self.right_panel
        )
        ...
        self.large_preview_label.setStyleSheet(
            "background-color: #f9f9f9; border: 2px dashed #ccc; font-size: 14pt; color: #999;"
        )
    ...
    def _initialize_page_order(self):
        ...
        for i, page_path in enumerate(self.current_group):
            metadata = next(
                (m for m in self.page_metadata_list if m["image_path"] == page_path), None
            )
    ...
    def _auto_reorder_pages(self):
        ...
        pages_with_numbers = [
            m for m in self.page_metadata_list if m.get("detected_page_number") is not None
        ]
    ...
    def _reset_page_order(self):
        ...
        reply = show_question(
            self, "Reset Order", "Reset to original page order from stitching step?"
        )
    ...
    def _on_approve_page_order(self):
        ...
        self.status_label.setText(
            f"Page order approved. {len(self.current_group)} pages ready for PDF."
        )
    ...
    def _on_back_to_step1(self):
        ...
        reply = show_question(
            self, "Return to Stitching?", "Going back will discard any metadata edits. Continue?"
        )
    ...
    def _start_content_based_ordering(self):
        ...
        self.worker_thread = OllamaWorker(
            self.ollama_service.infer_page_order_from_content, selected_model, self.current_group
        )
    ...
    def _setup_step4_ui(self):
        ...
        self.accept_delete_button.clicked.connect(
            lambda: self._finalize_document(delete_sources=True)
        )
        ...
        self.accept_keep_button.clicked.connect(
            lambda: self._finalize_document(delete_sources=False)
        )
    ...
    def _update_connection_status(self):
        ...
        try:
            ...
            self.connection_status_label.setToolTip(
                "Cannot connect to Ollama - check if service is running"
            )
    ...
    def _on_status_clicked(self, event):
        ...
        if self.last_ollama_response is not None or self.last_ollama_request is not None:
            ...
            info_label = QLabel(
                f"<b>Model:</b> {model_name} | <b>Type:</b> {self.last_ollama_response_type}"
            )
    ...
    def _scan_and_group(self):
        ...
        try:
            ...
            self.all_files = self.file_service._get_image_files()
            ...
            if not selected_model:
                show_warning(
                    self, "No Model Selected", "Please select a model from the dropdown above."
                )
            ...
            if not self._check_ollama_connection():
                self.status_label.setText(
                    "Ollama connection failed. Please check the error message."
                )
                # Return to loading UI
                return
    ...
    def _load_next_page_for_stitching(self):
        ...
        if not self.current_group:
            ...
            self.status_label.setText(
                f"Started new group. Page 1 added. ({files_remaining} files remaining)"
            )
        ...
        self._start_spinner()
        self._set_status_with_time(
            f"Analyzing page {group_size + 1} with {selected_model}... ({files_remaining} files remaining)"
        )
        ...
        if hasattr(self, "step1_spinner"):
            ...
            self.step1_spinner.setToolTip(
                f"Click to copy prompt to clipboard\n\nPrompt:\n{prompt_preview}..."
            )
...
{file_list}"""
        ...
        self.worker_thread = OllamaWorker(
            self.ollama_service.validate_grouping_with_page_number,
            selected_model,
            files_to_validate,
            pages_prompt,
        )
        self.worker_thread.finished.connect(
            lambda result: self._on_page_validation_result(result, next_file, start_time=start_time)
        )
        self.worker_thread.progress.connect(self._on_worker_progress)
        self.worker_thread.start()
    ...
    def _on_include_current_page(self):
        ...
        if current_state == "excluded":
            ...
            self.status_label.setText(
                f"Page marked as included. Group has {len(self.current_group)} page(s)."
            )
            ...
            files_remaining = len(self.all_files) - self.current_file_index
            self.status_label.setText(
                f"✓ Page included. Group has {len(self.current_group)} page(s). ({files_remaining} remaining)"
            )
    ...
    def _on_gallery_image_toggled(self, file_path: str, checked: bool):
        ...
        if checked:
            ...
            if file_path not in self.current_group:
                ...
                self.status_label.setText(
                    f"✓ {os.path.basename(file_path)} added to group. Group has {len(self.current_group)} page(s)."
                )
        else:
            ...
            if file_path in self.current_group:
                ...
                self.status_label.setText(
                    f"✗ {os.path.basename(file_path)} removed from group. Group has {len(self.current_group)} page(s)."
                )
    ...
    def _on_finish_group(self):
        ...
        self.status_label.setText(
            f"Document stitching complete. {len(self.current_group)} page(s) in group. Moving to analysis..."
        )
    ...
    def _on_cache_indicator_clicked(self):
        ...
        self.page_metadata_list = [
            m for m in self.page_metadata_list if m["image_path"] != current_page
        ]
        ...
        self.status_label.setText(
            f"Requesting fresh analysis from Ollama for {os.path.basename(current_page)}..."
        )
        self._load_next_page_for_stitching()
    ...
    def _display_page_in_large_preview(self, image_path, show_indicator=True):
        ...
        if pixmap.isNull():
            ...
            self.large_preview_label.setStyleSheet(
                "background-color: #ffe6e6; border: 2px solid #ccc;"
            )
            return
        ...
        self.large_preview_label.setStyleSheet(
            "background-color: #ffffff; border: 2px solid #2563EB;"
        )
        ...
        if show_indicator and self.current_step == WorkflowStep.STITCHING:
            ...
            self.preview_overlay.setGeometry(self.large_preview_label.width() - 60, 10, 50, 50)
            ...
            if state == "included":
                ...
                self.preview_overlay.mousePressEvent = lambda event: (
                    self._on_status_indicator_clicked(image_path, "included")
                )
            elif state == "excluded":
                self.preview_overlay.setText("✗")
                self.preview_overlay.setStyleSheet(
                ...
                self.preview_overlay.mousePressEvent = lambda event: (
                    self._on_status_indicator_clicked(image_path, "excluded")
                )
            else:  # pending
                self.preview_overlay.setText("?")
                self.preview_overlay.setStyleSheet(
            ...
            page_meta = next(
                (m for m in self.page_metadata_list if m["image_path"] == image_path), None
            )
            ...
            if from_cache:
                ...
                self.cache_indicator_label.setToolTip(
                    "Decision from CACHE - Click to refresh from Ollama"
                )
    ...
    def _start_metadata_extraction(self):
        ...
        title_keywords = self.config_manager.get_setting(
            "DocumentProcessing", "title_keywords", "Invoice, Statement, Bill"
        )
        ...
        file_list = "\n".join(
            f"  {i}. {path} (exists: {os.path.exists(path)})"
...
{file_list}"""
    ...
    def _on_abort(self):
        ...
        reply = show_question(
            self, "Abort Processing", "Are you sure you want to abort? All progress will be lost."
        )
        if reply == QMessageBox.StandardButton.Yes:
            self.close()
    ...
    def _create_pdf_for_preview(self):
        ...
        try:
            ...
            self.created_pdf_path = self.file_service.create_searchable_pdf(
                self.current_group,
                output_filename,
    ...
    def _update_step4_file_info(self):
        ...
        pdf_link.linkActivated.connect(
            lambda: QDesktopServices.openUrl(QUrl.fromLocalFile(self.created_pdf_path))
        )
        ...
        folder_link.linkActivated.connect(
            lambda: QDesktopServices.openUrl(QUrl.fromLocalFile(output_folder))
        )
    ...
    def _on_reject_pdf(self):
        ...
        reply = show_question(
            self, "PDF Rejected", "PDF has been rejected. Start over with this document?"
        )
    ...
    def _on_analysis_progress(self, status_text, current, total):
        ...
        if hasattr(self, "status_label") and self.status_label:
            self.status_label.setText(f"Analyzing files: {current}/{total} - {status_text}")
...
class StartupWindow(QWidget):
        ...
        if os.path.exists(scanner_gif_path):
            ...
            if self.movie.isValid():
                ...
                if original_size.width() > 0 and original_size.height() > 0:
                    ...
                    max_dimension = 350
                    if (
                        original_size.width() > max_dimension
                        or original_size.height() > max_dimension
                    ):
                        scale_factor = min(
                            max_dimension / original_size.width(),
                            max_dimension / original_size.height(),
                        )
                        scaled_size = QSize(
    ...
    def _process_pdfs(self):
        ...
        pdf_files, _ = QFileDialog.getOpenFileNames(
            self, "Select PDF Files to Convert", "", "PDF Files (*.pdf);;All Files (*.*)"
        )
    ...
    def run(self):
        ...
        try:
            ...
            stats = analysis_service.scan_all_directories(
                progress_callback=progress_callback, incremental=True, abort_check=abort_check
            )
