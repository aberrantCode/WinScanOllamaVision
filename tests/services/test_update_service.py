...
def _make_release(
    tag_name: str, prerelease: bool = False, assets: list[dict[str, Any]] | None = None
) -> FetchedRelease:
    return FetchedRelease(tag_name=tag_name, prerelease=prerelease, assets=assets or [])
...
def test_newer_stable_tag_triggers_update():
    rel = _make_release(tag_name="v1.2.3", assets=[_installer_asset("1.2.3")])
...
def test_same_tag_returns_none():
    rel = _make_release(tag_name="v1.0.0", assets=[_installer_asset("1.0.0")])
    assert decide_update("1.0.0", rel, False, "") is None


def test_older_tag_returns_none():
    rel = _make_release(tag_name="v0.9.0", assets=[_installer_asset("0.9.0")])
    assert decide_update("1.0.0", rel, False, "") is None


def test_prerelease_filtered_by_default():
    rel = _make_release(tag_name="v1.2.3rc1", prerelease=True)
    assert decide_update("1.0.0", rel, include_prereleases=False, skipped="") is None


def test_prerelease_allowed_when_opted_in():
    rel = _make_release(
        tag_name="v1.2.3rc1", prerelease=True, assets=[_installer_asset("1.2.3rc1")]
    )
...
def test_no_installer_asset_returns_info_with_empty_url():
    ...
    as not-actionable."""
    rel = _make_release(tag_name="v1.2.3", assets=[{"name": "source.tar.gz"}])
